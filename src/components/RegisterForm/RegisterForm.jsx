...
import { StyledWrapper, ModalWrapper, InputField, Button, Title, ImageReg, BottomBgImage, Link, InputWrapper, IconWrap, StateInputIcon } from './RegisterForm.styled';
			...
			<Formik initialValues={initialValues} validationSchema={registerSchema} onSubmit={handleSubmit}>
					{
						({ values, errors, touched, isSubmitting, handleChange }) => (
							<Form autoComplete='off'>
								<InputWrapper>
									<Field as={InputField} className={`sign-up__inp ${errors.name && touched.name ? "sign-up__inp_error" : ""}`} type="text" name="name" placeholder="name" values={values.name} />
									<IconWrap>
										{window.innerWidth < 768 && <AiOutlineUser color={`${errors.password && touched.password ? "red" : "white"}`} size={20} />}
										{window.innerWidth >= 768 && <AiOutlineUser color={`${errors.password && touched.password ? "red":"white"}`} size={24} />}
									</IconWrap>
									{(errors.name && touched.name) && <StateInputIcon src={errorIcon} />}
									<FormError name="name" component="div" />
								</InputWrapper>
								<InputWrapper>
									<Field as={InputField} className={`sign-up__inp ${errors.email && touched.email ? "sign-up__inp_error" : ""}`} name="email" type="email" inputmode="email" placeholder="email" values={values.email}/>
									<IconWrap>
								...
								<InputWrapper>
									<Field as={InputField} className={`sign-up__inp ${errors.password && touched.password ? "sign-up__inp_error" : ""} ${(0 < values.password.length && values.password.length < 8) && !touched.password ? "sign-up__inp_war" : ""} ${(8 <= values.password.length) && !touched.password ? "sign-up__inp_suc" : ""}` } type="password"  name="password" placeholder="password" />
									<IconWrap>
